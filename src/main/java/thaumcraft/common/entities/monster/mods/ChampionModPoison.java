...
public class ChampionModPoison implements IChampionModifierEffect
{
    ...
    public void showFX(LivingEntity boss) {
        ...
        FXDispatcher.INSTANCE.drawGenericParticles(boss.getBoundingBox().minX + w, boss.getBoundingBox().minY + h, boss.getBoundingBox().minZ + d, 0.0, 0.02, 0.0, 0.2f, 0.6f + boss.level().getRandom().nextFloat() * 0.1f, 0.2f + boss.level().getRandom().nextFloat() * 0.1f, 0.7f, false, 579, 4, 1, 8 + boss.level().getRandom().nextInt(4), 0, 0.5f + boss.level().getRandom().nextFloat() * 0.2f, 0.5f, 0);
    }
