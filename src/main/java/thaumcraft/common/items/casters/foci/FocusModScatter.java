        ...
        if (getParent().supplyTrajectories() != null) {
            for (Trajectory sT : getParent().supplyTrajectories()) {
                for (int a = 0; a < forks; ++a) {
                    Vec3 sV = sT.source;
                    Vec3 dV = sT.direction;
                    dV = dV.normalize();
                    dV = dV.add(getPackage().world.getRandom().nextGaussian() * 0.007499999832361937 * angle, getPackage().world.getRandom().nextGaussian() * 0.007499999832361937 * angle, getPackage().world.getRandom().nextGaussian() * 0.007499999832361937 * angle);
