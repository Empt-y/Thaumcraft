...
public class MeshLoader
{
    ...
    private void addFace(String line) {
        ...
        if (args.length < 3 || args.length > 4) {
            throw new UnsupportedOperationException("faces must have 3 or 4 vertices");
        }
    ...
    private void loadMaterialLibrary(Identifier locOfParent, String path) throws IOException {
        String prefix = locOfParent.getPath();
        int pp = prefix.lastIndexOf(47);
        prefix = ((pp >= 0) ? prefix.substring(0, pp + 1) : "");
        Identifier loc = Identifier.fromNamespaceAndPath(locOfParent.getNamespace(), prefix + path);
        currentMatLib.loadFromStream(loc);
    }

    public MeshModel loadFromResource(Identifier loc) throws IOException {
        net.minecraft.server.packs.resources.Resource res = Minecraft.getInstance().getResourceManager().getResource(loc).orElseThrow();
