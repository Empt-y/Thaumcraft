...
public class AspectList implements Serializable {
    ...
    {
        aspects.clear();
        ListTag tlist = nbttagcompound.getListOrEmpty("Aspects");
		for (int j = 0; j < tlist.size(); j++) {
			CompoundTag rs = tlist.getCompoundOrEmpty(j);
			if (rs.contains("key")) {
				add(	Aspect.getAspect(rs.getStringOr("key", "")),
						rs.getIntOr("amount", 0));
			}
		}
    }

	public void loadAdditional(CompoundTag nbttagcompound, String label)
    {
        aspects.clear();
        ListTag tlist = nbttagcompound.getListOrEmpty(label);
		for (int j = 0; j < tlist.size(); j++) {
			CompoundTag rs = tlist.getCompoundOrEmpty(j);
			if (rs.contains("key")) {
				add(	Aspect.getAspect(rs.getStringOr("key", "")),
	...
	public void saveAdditional(CompoundTag nbttagcompound, String label)
    {
        ListTag tlist = new ListTag();
