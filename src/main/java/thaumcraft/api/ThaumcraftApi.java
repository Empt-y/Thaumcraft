...
public class ThaumcraftApi {
	...
	public static class BluePrint implements IThaumcraftRecipe{
		...
		// --- Recipe<CraftingInput> stubs: BluePrint is display-only, not a grid recipe ---

		@Override
		public boolean matches(CraftingInput inv, Level world) { return false; }

		@Override
		public ItemStack assemble(CraftingInput inv) { return ItemStack.EMPTY; }
	...
	public static InfusionRecipe getInfusionRecipe(ItemStack res) {
		for (Object r:getCraftingRecipes().values()) {
			if (r instanceof InfusionRecipe ir) {
				if (ir.getRecipeOutput() instanceof ItemStack is && ItemStack.isSameItemSameComponents(is, res))
					return ir;
			}
		}
		return null;
	...
	public static CrucibleRecipe getCrucibleRecipe(ItemStack stack) {
		for (Object r:getCraftingRecipes().values()) {
			if (r instanceof CrucibleRecipe) {
				if (ItemStack.isSameItemSameComponents(((CrucibleRecipe)r).getResultItem(), stack))
	 ...
	 * @param item the item passed. Pass 32767 if all damage values of this item/block should have the same aspects
	 ...
	 * @param item, pass 32767 to meta if all damage values of this item/block should have the same aspects
	 * @param aspects A ObjectTags object of the associated aspects
	 */
	@Deprecated
	 ...
	 * 	<br>ThaumcraftApi.registerEntityTag("Skeleton", (new AspectList()).add(Aspect.DEATH, 8), net.minecraft.nbt.ByteTag.valueOf("SkeletonType",(byte) 1));
	 */
	@Deprecated
	 ...
	 * the crop when fully grown. Sending a metadata of [32767] will mean the metadata won't get 
	 * checked.
	 * Example for vanilla wheat: 
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestStandardCrop", new ItemStack(Block.crops, 1));
	 *  
	 ...
	 * Sending a metadata of [32767] will mean the metadata won't get checked.
	 * Example (this will technically do nothing since clicking wheat does nothing, but you get the idea): 
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestClickableCrop", new ItemStack(Block.crops, 1));
	 * 
	 * Stacked crops (like reeds) are crops that you wish the bottom block should remain after harvesting.
	 * As for standard crops, you need to create and ItemStack that tells the golem what block id 
	 * and metadata represents the crop when fully grown. Sending a metadata of [32767] will mean the actualy md won't get 
	 * checked. If it has the order upgrade it will only harvest if the crop is more than one block high.
	 * Example: 
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestStackedCrop", new ItemStack(Block.reed, 1));
	 */
	 ...
	 * Sending a metadata of [32767] will mean the metadata won't get checked.
	 * Example for vanilla wheat: 
	 * FMLInterModComms.sendMessage("Thaumcraft", "lampBlacklist", new ItemStack(Block.crops.asItem(), 1));
	 */
