...
public class Quat implements Copyable<Quat>
{
    ...
    public Quat setAroundAxis(double ax, double ay, double az, double angle) {
        angle *= 0.5;
        double d4 = Mth.sin(angle);
        return set(Mth.cos(angle), ax * d4, ay * d4, az * d4);
    }
    ...
    public void rotate(Vector3 vec) {
        ...
        double d2 = s * vec.getX() + y * vec.getZ() - z * vec.getY();
        double d3 = s * vec.getY() - x * vec.getZ() + z * vec.getX();
        double d4 = s * vec.getZ() + x * vec.getY() - y * vec.getX();
