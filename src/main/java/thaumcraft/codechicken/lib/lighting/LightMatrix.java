...
public class LightMatrix implements CCRenderState.IVertexOperation
{
    ...
    public void locate(BlockGetter a, int x, int y, int z) {
        access = a;
        pos.set(x, y, z);
        computed = 0;
        sampled = 0;
    ...
    public void sample(int i) {
        // getCombinedLight, getLightValue, getAmbientOcclusionLightValue all removed in modern MC
        // lighting pipeline stub: use defaults
        bSamples[i] = 0xF000F0;
        aSamples[i] = 1.0f;
    }
    ...
    public void sideSample(int side) {
        if ((computed & 1 << side) == 0x0) {
            ...
            for (int q = 0; q < 4; ++q) {
                ...
                // Minecraft.isAmbientOcclusionEnabled() removed; always use AO samples
